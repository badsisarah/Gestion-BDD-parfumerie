...
function getAllFlacon(): array {
    $conn = connexion();
    $sql = "
        SELECT f.flacon_volume, f.flacon_prix, f.flacon_id, f.parfum_id, p.parfum_nom
        FROM G09_flacon f
        JOIN G09_parfum p ON f.parfum_id = p.parfum_id
        ORDER BY f.flacon_id
    ";
    pg_prepare($conn, "get_all_flacons_with_parfum", $sql);
    $result = pg_execute($conn, "get_all_flacons_with_parfum", []);
